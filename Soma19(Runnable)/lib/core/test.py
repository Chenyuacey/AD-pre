...
from utils.cython_mask_3d import binary_mask_to_rle
from numpy import *
...
def im_detect_all(model, im, box_proposals=None, timers=None):
    ...
    inputs, im_scale = _get_blobs(im)

    if cfg.TEST.NEED_CROP:
        cls_boxes_total = [np.empty((0, 7), dtype=np.float32) for _ in range(cfg.MODEL.NUM_CLASSES)]
        masks_total = np.empty((0, 1, cfg.MRCNN.RESOLUTION,cfg.MRCNN.RESOLUTION,cfg.MRCNN.RESOLUTION), dtype=np.float32)
        boxes_total = np.empty((0, 6), dtype=np.float32)
        cls_segms_total = [[] for _ in range(cfg.MODEL.NUM_CLASSES)]
        orig_im = inputs['data'][0, 0, :, :, :]
        slices, height, width = inputs['im_info'][0].astype(np.int16)[:3]
        patch_size = cfg.TEST.IN_SIZE
        if slices < patch_size[0]: #for nuclei data
            pad_s = np.int((patch_size[0] - slices) / 2)
            pad_e = patch_size[0] - slices - pad_s
            orig_im = np.append(tile(orig_im[0, :, :], (pad_s, 1, 1)), orig_im, axis=0)
            orig_im = np.append(orig_im, tile(orig_im[-1, :, :], (pad_e, 1, 1)), axis=0)
            slices = patch_size[0]
        else:
            pad_s = 0
        ...
        for ss in sidx:
            for hs in hidx:
                for ws in widx:
                    input_cube = {}
                    input_cube['data'] = orig_im[ss: ss + patch_size[0], hs: hs + patch_size[1],
                                         ws: ws + patch_size[2]][np.newaxis, np.newaxis, :, :, :]
                    input_cube['im_info'] = np.array(input_cube['data'] .shape[-3:] + (1.0,))[np.newaxis, :]

                    # todo: only can return a small patch's result
                    #if cfg.PRM_ON:
                    #    res_prm = im_detect_bbox(model, input_cube, im_scale)
                    #    return res_prm

                    timers['im_detect_bbox'].tic()

                    scores, boxes, im_scale, blob_conv = im_detect_bbox(model, input_cube, im_scale)
                    ...
                    scores, boxes, cls_boxes, _ = box_results_with_nms_and_limit(scores, boxes)
                    timers['misc_bbox'].toc()

                    if pad_s:
                        off_set = np.array([0, 0, -pad_s, 0, 0, -pad_s], dtype=np.float32)
                        boxes += off_set
                        off_set = np.array([0, 0, -pad_s, 0, 0, -pad_s, 0], dtype=np.float32)
                        cls_boxes[1:] = [det_cls[:, ] + off_set for det_cls in cls_boxes[1:]]
                    ...
                    if cfg.MODEL.MASK_ON and boxes.shape[0] > 0:
                        ...
                        cls_segms = segm_results(cls_boxes, masks, boxes, im.shape[0], im.shape[1], im.shape[2])
                    ...
                    # update results with offset
                    off_set = np.array([ws, hs, ss, ws, hs, ss, 0], dtype=np.float32)
                    cls_boxes[1:] = [det_cls[:, ] + off_set for det_cls in cls_boxes[1:]] # a list, each item is a nd array
                                                                                          # for one class with size: num_det, 7
                    boxes_total = np.append(boxes_total, boxes + off_set[:6], axis=0)
                    cls_boxes_total[1:] = [np.append(det_cls_total, det_cls, axis=0)
                                       for (det_cls_total, det_cls) in zip(cls_boxes_total[1:], cls_boxes[1:] )]
                    if cfg.MODEL.MASK_ON and boxes.shape[0] > 0:
                        masks_total = np.append(masks_total, masks, axis=0)


        for i in range(1, cfg.MODEL.NUM_CLASSES):
            dets_i = cls_boxes_total[i]
            if cfg.TRAIN.DATASETS == ('nuclei_det_seg_train',):
                condition1 = ((dets_i[:, 0] > 10) & (dets_i[:, 3] < width-10)) & ((dets_i[:, 3] - dets_i[:, 0] + 1) < 30)
                condition2 = ((dets_i[:, 1] > 10) & (dets_i[:, 4] < height-10)) & ((dets_i[:, 4] - dets_i[:, 1] + 1) < 30)
                keep = (condition1 | condition2)
                keep = ~keep
                dets_i = dets_i[keep, :]

            keep = box_utils_3d.nms_3d(dets_i, cfg.TEST.NMS)
            cls_boxes_total[i] = dets_i[keep, :]
            if cfg.MODEL.MASK_ON and masks_total.shape[0] > 0:
                boxes_total = boxes_total[keep, :]
                masks_total = masks_total[keep, :]
        if cfg.MODEL.MASK_ON:
            timers['misc_mask'].tic()
            cls_segms_total = segm_results(cls_boxes_total, masks_total, boxes_total, im.shape[0], im.shape[1],
                                                                                               im.shape[2])
            timers['misc_mask'].toc()
            timers['binary_mask_to_rle'].tic()
            for i in range(1, cfg.MODEL.NUM_CLASSES):
                for j in range(len(cls_segms_total[i])):
                   cls_segms_total[i][j] = binary_mask_to_rle(cls_segms_total[i][j])
            timers['binary_mask_to_rle'].toc()



    return cls_boxes_total, cls_segms_total, cls_keyps
...
def im_detect_bbox(model, inputs, im_scale, boxes=None):# target_scale, target_max_size,
    """Prepare the bbox for testing"""

    #, target_scale, target_max_size)
    input_shape = inputs['im_info'][0][:3]
    ...
    if cfg.MODEL.RPN_ONLY:
        return scores, boxes, im_scale, return_dict['blob_conv']
    ...
    if cfg.TEST.BBOX_REG:
        ...
        if cfg.MODEL.CLS_AGNOSTIC_BBOX_REG: # false
            # Remove predictions for bg class (compat with MSRA code)
            box_deltas = box_deltas[:, -6:]
        if cfg.TRAIN.BBOX_NORMALIZE_TARGETS_PRECOMPUTED:
            # (legacy) Optionally normalize targets by a precomputed mean and stdev
            box_deltas = box_deltas.view(-1, 6) * cfg.TRAIN.BBOX_NORMALIZE_STDS \
...
def im_detect_bbox_aspect_ratio(
    ...
    if not cfg.MODEL.FASTER_RCNN:
        box_proposals_ar = box_utils_3d.aspect_ratio(box_proposals, aspect_ratio)
    else:
    ...
    boxes_inv = box_utils_3d.aspect_ratio(boxes_ar, 1.0 / aspect_ratio)
...
def im_detect_mask(model, im_scale, boxes, blob_conv):
    ...
    if cfg.MRCNN.CLS_SPECIFIC_MASK:
        pred_masks = pred_masks.reshape([-1, cfg.MODEL.NUM_CLASSES, M, M, M])
    else:
        pred_masks = pred_masks.reshape([-1, 1, M, M, M])
...
def box_results_with_nms_and_limit(scores, boxes, scores_keep_idx=None):  # NOTE: support single-batch
    ...
    cls_keep_idx = [[] for _ in range(num_classes)]
    # Apply threshold on detection probabilities and apply NMS
    # Skip j = 0, because it's the background class
    for j in range(1, num_classes):
        if cfg.MODEL.RPN_ONLY:
            inds = np.where(scores > cfg.TEST.SCORE_THRESH)[1]
            scores_j = scores[0, inds]
            boxes_j = boxes[inds, :]
            if not scores_keep_idx is None:
                keep_idx_j = scores_keep_idx[inds].copy()
        else:
        ...
        if cfg.TEST.SOFT_NMS.ENABLED: # false Note: don't care score_keep_idx
            nms_dets, _ = box_utils_3d.soft_nms_3d(
                dets_j,
            ...
            keep = box_utils_3d.nms_3d(dets_j, cfg.TEST.NMS)
            nms_dets = dets_j[keep, :]
            if not scores_keep_idx is None:
                keep_idx_j = keep_idx_j[keep]

        # Refine the post-NMS boxes using bounding-box voting
        if cfg.TEST.BBOX_VOTE.ENABLED: # false
            nms_dets = box_utils_3d.box_voting(
                nms_dets,
                dets_j,
    ...
    if cfg.TEST.DETECTIONS_PER_IM > 0:
        ...
        if len(image_scores) > cfg.TEST.DETECTIONS_PER_IM:
            ...
            for j in range(1, num_classes):
                ...
                print(cls_keep_idx.shape)
                cls_keep_idx[j] = cls_keep_idx[j][keep, :]

    im_results = np.vstack([cls_boxes[j] for j in range(1, num_classes)])
    boxes = im_results[:, :-1]
    scores = im_results[:, -1]
    return scores, boxes, cls_boxes, cls_keep_idx


def segm_results(cls_boxes, masks, ref_boxes, im_s, im_h, im_w):
    num_classes = cfg.MODEL.NUM_CLASSES
    ...
    M = cfg.MRCNN.RESOLUTION
    scale = (M + 2.0) / M
    ref_boxes = box_utils_3d.expand_boxes(ref_boxes, scale)
    ...
    for j in range(1, num_classes):
        ...
        for _ in range(cls_boxes[j].shape[0]):
            if cfg.MRCNN.CLS_SPECIFIC_MASK:
                ...
                padded_mask[1:-1, 1:-1, 1:-1] = masks[mask_ind, 0, :, :, :]

            ref_box = ref_boxes[mask_ind, :]
            w = (ref_box[3] - ref_box[0] + 1)
            h = (ref_box[4] - ref_box[1] + 1)
            s = (ref_box[5] - ref_box[2] + 1)
            w = np.maximum(w, 1)
            h = np.maximum(h, 1)
            s = np.maximum(s, 1)

            # mask = cv2.resize(padded_mask, (w, h))
            mask = transform.resize(padded_mask, (s, h, w), mode='reflect', anti_aliasing=True)
            mask = np.array(mask > cfg.MRCNN.THRESH_BINARIZE, dtype=np.uint8)
            im_mask = np.zeros((im_s, im_h, im_w), dtype=np.uint8)
            ...
            z_0 = max(ref_box[2], 0)
            z_1 = min(ref_box[5] + 1, im_s)

            im_mask[z_0:z_1, y_0:y_1, x_0:x_1] = mask[(z_0 - ref_box[2]):(z_1 - ref_box[2]),
            ...
            segms.append(im_mask) # a list whose item is np.array
...
def _get_blobs(im, rois=None):#, target_scale, target_max_size):
