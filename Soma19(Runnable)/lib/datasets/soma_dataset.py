...
class SomaDataset(object):
    ...
    def _load_ann_objs(self, file_name):
        ...
        for a in ann:
            ...
            #radius = int(part_a[3])
            ###add by zhiyi###
            radius_xy = int(part_a[3])
            radius_z = int(part_a[4])
            ###add by zhiyi###
            obj = {}
            #expand_dimeter = radius*2*(1. + cfg.TRAIN.RADIUS_EXP_RATIO)
            ###add by zhiyi###
            expand_dimeter_xy = radius_xy*2*(1. + cfg.TRAIN.RADIUS_EXP_RATIO)
            expand_dimeter_z = radius_z*2*(1. + cfg.TRAIN.RADIUS_EXP_RATIO)
            ###add by zhiyi###
            ...
            ###add by zhiyi###
            x1 = int(np.maximum(pos_x - expand_dimeter_xy / 2.0, 0.0))
            y1 = int(np.maximum(pos_y - expand_dimeter_xy / 2.0, 0.0))
            z1 = int(np.maximum(pos_z - expand_dimeter_z / 2.0, 0.0))
            w = int(np.minimum(pos_x + expand_dimeter_xy / 2.0, width - 1)) - x1 + 1
            h = int(np.minimum(pos_y + expand_dimeter_xy / 2.0, height - 1)) - y1 + 1
            s = int(np.minimum(pos_z + expand_dimeter_z / 2.0, slices - 1)) - z1 + 1
            ###add by zhiyi###
            ...
            ###add by zhiyi###
            obj['segmentation'] = [pos_x, pos_y, pos_z, radius_xy,radius_z]
            obj['volume'] = 4. / 3. * Pi * radius_xy * radius_xy * radius_z
            ###add by zhiyi###
            obj['iscrowd'] = 0
            obj['ignore'] = 0
            objs.append(obj)
